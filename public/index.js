...
async function initGNMathGames(container) {
    try {
        ...
        zones.forEach(file => {
            ...
            btn.onclick = async () => {
                const html = await fetch(file.url.replace("{HTML_URL}", "https://cdn.jsdelivr.net/gh/gn-math/html@main")).then(r => r.text());
        ...
        container.innerHTML = "Failed to load games: " + e.message;
    }
}
