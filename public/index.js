...
let currentBg = 0;
const backgrounds = [
  "https://images.unsplash.com/photo-1514897575457-c4db467cf78e?q=80&w=1170&auto=format&fit=crop", // space
  "https://images.unsplash.com/photo-1612832020620-d12504dbb7dc?q=80&w=1170&auto=format&fit=crop"  // beach
];

bgSwitcher.addEventListener("click", () => {
  currentBg = (currentBg + 1) % backgrounds.length;
  document.body.style.background = `url('${backgrounds[currentBg]}') center/cover no-repeat fixed`;
});
...
async function openWindow(appId) {
  ...
  /* ===================== APPS ===================== */
  if (appId === "browser") await initCustomBrowser(content, "https://search.brave.com/");
  ...
  if (appId === "movies") await initCustomBrowser(content, "https://www.cineby.gd/");
}
...
function updateTaskbar() {
  ...
  Object.keys(windows).forEach(appId => {
    ...
    btn.onclick = () => { windows[appId].style.display = "flex"; windows[appId].style.zIndex = Date.now(); };
...
/* ===================== CUSTOM BROWSER USING PROXY ===================== */
async function initCustomBrowser(container, url) {
  container.innerHTML = "";
...
async function openGNGame(url, contentDiv) {
  ...
  frame.src = url.replace("{HTML_URL}", "https://cdn.jsdelivr.net/gh/gn-math/html@main");
